...
email_service = EmailService()  # Now uses AWS SES in production
...
async def register_user(user_data: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        error_message = str(e)
        logger.error(f"Cognito signup error for {user_data.email}: {error_message}")
        
        # Handle common Cognito errors
        if "UsernameExistsException" in error_message or "AliasExistsException" in error_message:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Email already registered"
            )
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid user parameters"
            )
        else:
            raise HTTPException(
...
async def login_user(login_data: UserLogin, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        cognito_tokens = auth_response["tokens"]
        
        # Get user from database (for logging and compatibility)
        from sqlalchemy import func
        result = await db.execute(select(User).where(func.lower(User.email) == login_data.email.lower()))
        user = result.scalar_one_or_none()
        
        # If user doesn't exist in our DB, create a placeholder record (optional)
        # This ensures compatibility with existing code that expects a user record
        if not user:
            logger.warning(f"User {login_data.email} authenticated in Cognito but not found in local database")
            # Optionally create a minimal user record here if needed
            # For now, we'll just log and continue
        
        # Log login if user exists in DB
        if user:
        ...
        # Return Cognito tokens in the same format as before
        return Token(
        ...
        if "NotAuthorizedException" in error_message or "Invalid email or password" in error_message:
            ...
            raise HTTPException(
                ...
                detail="Email not verified. Please confirm your email address first."
            )
        ...
        elif "TooManyFailedAttemptsException" in error_message:
            raise HTTPException(
...
async def request_otp(otp_request: OTPRequest, db: AsyncSession = Depends(get_db)):
    ...
    For email_verification: Resends confirmation code via AWS Cognito
    For password_reset: Uses existing database OTP logic (backward compatibility)
    ...
    if otp_request.purpose == "email_verification":
        # Use Cognito to resend confirmation code
        cognito_service = get_cognito_service()
        try:
            cognito_service.resend_confirmation_code(email=otp_request.email)
            logger.info(f"Confirmation code resent via Cognito: {otp_request.email}")
            return MessageResponse(message="OTP sent successfully")
        except Exception as e:
            error_message = str(e)
            logger.error(f"Cognito resend_confirmation_code error for {otp_request.email}: {error_message}")
        ...
        # Use existing database OTP logic for password reset
...
async def verify_otp(otp_data: OTPVerify, db: AsyncSession = Depends(get_db)):
    """
    Confirm signup using OTP code from AWS Cognito
    
    Verifies the OTP code sent to user's email via Cognito and confirms the user signup.
    
    - **email**: Email address that received the OTP
    - **code**: 6-digit OTP code from Cognito
    - **purpose**: Purpose of the OTP verification (email_verification, password_reset)
    
    For email_verification: Uses Cognito confirm_sign_up
    For password_reset: Uses existing database OTP logic (backward compatibility)
    """
    ...
    logger.info(f"OTP verification attempt for {otp_data.email}: code='{normalized_code}', purpose={otp_data.purpose}")
    
    # Handle email verification (signup confirmation) via Cognito
    if otp_data.purpose == "email_verification":
        cognito_service = get_cognito_service()
        
        try:
            # Check for developer OTP bypass (for testing)
            if (str(normalized_code) == str(developer_otp) or 
                normalized_code.strip() == developer_otp.strip() or 
                normalized_code == "123456"):
                logger.info(f"✅ Developer OTP bypass used for {otp_data.email}")
                # Skip Cognito confirmation for developer OTP, just mark as verified in DB
                ...
                if user:
                    ...
                    await db.commit()
                    logger.info(f"✅ User {otp_data.email} marked as verified (developer bypass)")
                return MessageResponse(message="Signup complete")
            
            # Confirm signup in Cognito using OTP
            cognito_service.confirm_sign_up(
                email=otp_data.email,
                confirmation_code=normalized_code
            )
            
            # Update user in database to mark as verified
            user_result = await db.execute(select(User).where(User.email == otp_data.email))
            user = user_result.scalar_one_or_none()
            if user:
                user.email_verified = True
                await log_user_action(db, str(user.id), "email_verified", "user", str(user.id))
                await db.commit()
                logger.info(f"✅ User {otp_data.email} confirmed in Cognito and marked as verified")
            
            return MessageResponse(message="Signup complete")
            
        except Exception as e:
            error_message = str(e)
            logger.error(f"Cognito confirm_sign_up error for {otp_data.email}: {error_message}")
            
            # Handle common Cognito errors
            if "CodeMismatchException" in error_message or "Invalid code" in error_message.lower():
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Invalid confirmation code. Please check your OTP and try again."
                )
            elif "ExpiredCodeException" in error_message or "expired" in error_message.lower():
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Confirmation code has expired. Please request a new OTP."
                )
            elif "UserNotFoundException" in error_message:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
    ...
    # Handle password reset (keep existing database logic for backward compatibility)
    else:
        # Check for developer OTP bypass
        if (str(normalized_code) == str(developer_otp) or 
            normalized_code.strip() == developer_otp.strip() or 
            normalized_code == "123456"):
            logger.info(f"✅ Developer OTP bypass used for password reset: {otp_data.email}")
...
async def send_verification_otp(email: str, db: AsyncSession):
    ...
    otp = OTP(
        ...
        expires_at=datetime.utcnow() + timedelta(minutes=5)  # 5-minute expiry as required
    )
    db.add(otp)
...
async def send_password_reset_otp(email: str, db: AsyncSession):
    ...
    # Generate 6-digit OTP (random and unpredictable)
    otp_code = generate_otp(length=6)
