...
class T1FormStorageService {
  ...
  /// NOTE: This saves to local storage only. For database persistence,
  /// use T1FormApi.submitForm() which saves directly to the backend database.
  Future<bool> saveForm(T1FormData form) async {
    try {
      ...
      _logger.e('Error saving form locally: $e');
      return false;
    }
  }
  
  /// Save form directly to database (not just local storage)
  /// This is the preferred method - saves to database immediately
  Future<bool> saveFormToDatabase(T1FormData form) async {
    try {
      // Import T1FormApi dynamically to avoid circular dependencies
      // For now, we'll call it from the UI layer
      // This method is kept for future use if needed
      _logger.w('saveFormToDatabase called - use T1FormApi.submitForm() directly instead');
      return false;
    } catch (e) {
      _logger.e('Error saving form to database: $e');
      return false;
    }
  }
