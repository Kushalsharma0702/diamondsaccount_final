...
async def create_payment(
    ...
    # Notify client (stored in shared Postgres notifications table)
    try:
        await db.execute(
            text(
                """
                INSERT INTO notifications (id, user_id, type, title, message, payload, is_read, created_by, created_at)
                VALUES (:id, :user_id, :type, :title, :message, :payload::jsonb, false, 'admin', NOW())
                """
            ),
            {
                "id": str(uuid.uuid4()),
                ...
                "message": f"A payment of ${payment.amount} was recorded.",
                "payload": '{"source":"admin","kind":"payment","action":"created"}',
            },
        )
        await db.commit()
    except Exception:
        # Don't fail payment flow if notification insert fails
        pass
...
async def update_payment(
    ...
    # Notify client if amount/method/note changed
    try:
        await db.execute(
            text(
                """
            ...
            ),
            {
                ...
                "type": "payment",
                "title": "Payment updated",
                "message": "Your payment information was updated by admin.",
                "payload": '{"source":"admin","kind":"payment","action":"updated"}',
            },
        )
        await db.commit()
    except Exception:
